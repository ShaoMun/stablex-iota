...
>(({ className, children, style }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "z-10 flex size-10 md:size-12 items-center justify-center rounded-full border-2 p-2",
        className
      )}
      style={{
        background: 'linear-gradient(135deg, #ffffff 0%, #c0c0c0 30%, #808080 50%, #606060 70%, #404040 100%)',
        borderColor: 'rgba(255, 255, 255, 0.3)',
        boxShadow: '0 0 20px -12px rgba(255, 255, 255, 0.3)',
        ...style,
      }}
    >
      {children}
...
export function AnimatedBeamDemo() {
  ...
  // Calculate positions for 7 nodes around center in a perfect circle
  const radius = 160;
  const nodePositions = Array.from({ length: 7 }, (_, i) => {
    const angle = (i * Math.PI * 2) / 7 - Math.PI / 2; // Start from top
    return {
  ...
  return (
    ...
    >
      <div className="relative w-full h-full" style={{ minHeight: '400px' }}>
        ...
        >
          <Circle 
            ...
            style={{
              ...
              boxShadow: '0 0 30px -10px rgba(255, 107, 107, 0.5), 0 0 20px -12px rgba(78, 205, 196, 0.4)',
            }}
          >
            {/* <Icons.openai /> */}
          </Circle>
        ...
        {/* 7 nodes around center in orbit */}
        {nodeRefs.map((nodeRef, index) => (
          <div
            key={index}
            className="absolute top-1/2 left-1/2"
            style={{
              transform: `translate(calc(-50% + ${nodePositions[index].x}px), calc(-50% + ${nodePositions[index].y}px))`,
            }}
            ref={nodeRef}
          >
            <Circle>
      ...
      {/* Flow pattern: Center → Node → Next Node (clockwise) → Center */}
      {/* Phase 1 (0-3s): Center → All Nodes */}
      {nodeRefs.map((nodeRef, index) => (
        <AnimatedBeam
          key={`center-to-${index}`}
          ...
          gradientStopColor="#808080"
          duration={3}
          delay={0}
        />
      ))}

      {/* Phase 2 (3-6s): Node → Next Node (clockwise) */}
      {nodeRefs.map((nodeRef, index) => {
        const nextIndex = (index + 1) % 7; // Next node clockwise
        const nextNodeRef = nodeRefs[nextIndex];
        return (
          <AnimatedBeam
