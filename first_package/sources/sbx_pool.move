module first_package::sbx_pool {
    ...
    public entry fun create_pool(admin: &signer, fee_bps: u64, min_reserve_bps: u64, ctx: &mut TxContext) {
    ...
    /// Admin controls
    public entry fun pause(pool: &mut Pool, admin: &signer, ctx: &TxContext) {
        assert!(pool.admin == tx_context::sender(ctx), E_NOT_ADMIN);
        pool.paused = true;
    }

    public entry fun unpause(pool: &mut Pool, admin: &signer, ctx: &TxContext) {
        assert!(pool.admin == tx_context::sender(ctx), E_NOT_ADMIN);
        pool.paused = false;
    }

    /// Initialize registry
    public entry fun create_registry(admin: &signer, pyth_state: address, pyth_package: address, ctx: &mut TxContext) {
    ...
    public entry fun admin_set_feeds(registry: &mut Registry, admin: &signer, xsgd_feed: address, myrc_feed: address, jpyc_feed: address, ctx: &TxContext) {
    ...
    public entry fun admin_set_whitelist(registry: &mut Registry, admin: &signer, xsgd: bool, myrc: bool, jpyc: bool, ctx: &TxContext) {
        assert!(registry.admin == tx_context::sender(ctx), E_NOT_ADMIN);
    ...
    public entry fun admin_set_prices_microusd(registry: &mut Registry, admin: &signer, xsgd: u64, myrc: u64, jpyc: u64, ctx: &TxContext) {
        assert!(registry.admin == tx_context::sender(ctx), E_NOT_ADMIN);
    ...
    public entry fun admin_deposit_usdc(pool: &mut Pool, admin: &signer, amount: u64, ctx: &TxContext) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(pool.admin == tx_context::sender(ctx), E_NOT_ADMIN);
        pool.usdc_reserve = pool.usdc_reserve + amount;
    }

    public entry fun admin_withdraw_usdc(pool: &mut Pool, admin: &signer, amount: u64, ctx: &TxContext) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(pool.admin == tx_context::sender(ctx), E_NOT_ADMIN);
    ...
    /// User operations
    public entry fun deposit_and_mint(account: &mut Account, pool: &mut Pool, user: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
    ...
    public entry fun redeem_regional(account: &mut Account, pool: &mut Pool, user: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(!pool.paused, E_PAUSED);
    ...
    public entry fun deposit_xsgd(account: &mut Account, pool: &mut Pool, registry: &Registry, user: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(!pool.paused, E_PAUSED);
    ...
    public entry fun deposit_myrc(account: &mut Account, pool: &mut Pool, registry: &Registry, user: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(!pool.paused, E_PAUSED);
    ...
    public entry fun deposit_jpyc(account: &mut Account, pool: &mut Pool, registry: &Registry, user: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
        assert!(!pool.paused, E_PAUSED);
    ...
    public entry fun instant_swap_to_usdc(account: &mut Account, pool: &mut Pool, user: &signer, sbx_amount: u64) {
    ...
    public entry fun transfer_sbx(from_account: &mut Account, to_account: &mut Account, sender: &signer, amount: u64) {
        assert!(amount > 0, E_ZERO_AMOUNT);
